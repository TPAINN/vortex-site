...
/**
 * Clean, premium preloader.
 *
 * Minimal layout: logo → wordmark → thin progress bar with a glowing tip.
 * The bar fill is a pure CSS animation (inline <style> tag — immune to
 * Tailwind tree-shaking). The numeric readout is driven by rAF via refs
 * (no setState → zero re-renders → animation never restarts).
 *
 * Exit: fades out + scales up subtly for a soft hand-off to the splash.
 */
...
/**
 * rAF-driven numeric readout (refs only, no setState).
 * Injects the CSS keyframe for the bar fill via inline <style>.
 */
