...
export default function Download() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus("done");
      // bump the live counter if the API returned the new total
      if (typeof data?.total === "number") setCount(data.total);
      else setCount((c) => (c != null ? c + 1 : c));
  ...
  return (
    <section id="download" className="relative overflow-hidden px-4 py-28 text-center sm:py-36">
      ...
      <div className="relative z-10 mx-auto flex w-[min(760px,100%)] flex-col items-center gap-6">
        ...
        <Reveal delay={0.13}>
          {count != null && (
            <div className="inline-flex items-center gap-2 rounded-full border border-glass bg-glass px-3.5 py-1.5 font-mono text-[11.5px] text-ink-2 backdrop-blur-md">
...
function CountUpLive({ value }: { value: number }) {
  ...
  return (
    <span ref={ref} className="font-semibold tabular-nums text-ink">
