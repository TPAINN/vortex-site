...
/**
 * The reveal curtain. The site sits underneath (fully rendered, opaque);
 * this panel slides up (translateY: -100%) when the user performs a
 * "go down" gesture:
 *   • mouse wheel down
 *   • touch swipe up
 *   • keyboard: ArrowDown / PageDown / Space / Enter
 *   • clicking the Enter button
 */
export default function SplashScreen({ onReveal }: SplashScreenProps) {
  const revealedRef = useRef(false);
  const [hintPulse, setHintPulse] = useState(0);

  // magnetic logo follow
  ...
  useEffect(() => {
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      if (startY.current != null && y != null) {
        const dy = startY.current - y; // up swipe => positive
        if (dy > 45) reveal();
      }
    };
