...
const STATS: Stat[] = [
  ...
  { value: 0, prefix: "$", label: "Cost · forever free" },
];
  ...
  return (
    <Reveal delay={index * 0.1} y={20}>
      <div className="group relative h-full overflow-hidden rounded-3xl border border-glass bg-glass p-7 backdrop-blur-md transition-colors hover:border-violet/40">
        <div className="pointer-events-none absolute -left-10 -bottom-10 h-32 w-32 rounded-full opacity-0 blur-3xl transition-opacity duration-500 group-hover:opacity-100"
...
export default function Stats() {
  ...
  return (
    <section ref={ref} className="relative px-4 py-16 sm:py-20">
      <div className="mx-auto grid w-[min(1120px,100%)] grid-cols-2 gap-4 lg:grid-cols-4">
        {STATS.map((s, i) => (
          <StatItem key={s.label} stat={s} run={inView} index={i} />
        ))}
      </div>
    </section>
  );
