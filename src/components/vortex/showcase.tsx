...
import { Check, Download, Music2, Video } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          boxShadow:
            "0 0 0 9px oklch(0.13 0.01 290), 0 0 0 10px oklch(1 0 0 / 0.08), 0 36px 90px oklch(0.55 0.25 293 / 0.28)",
        }}
        role="img"
      ...
      >
        ...
        <div className="flex flex-wrap gap-1.5">
          {["Best", "1080p", "720p"].map((c) => (
            <span key={c} className="rounded-full border border-glass px-2.5 py-1.5 font-mono text-[10.5px] text-ink-3">
              {c}
            </span>
          ))}
...
export default function Showcase() {
  return (
    <section id="app" className="relative px-4 py-24 sm:py-28">
      <div className="mx-auto grid w-[min(1120px,100%)] items-center gap-14 lg:grid-cols-[minmax(280px,380px)_1fr]">
        ...
        <div>
          ...
          <dl className="mt-10 flex flex-col gap-0">
            {SPECS.map((s, i) => (
              <Reveal key={s.k} delay={i * 0.06} y={18}>
...
/** Animated audio waveform — bars pulse with staggered phases. */
        ...
        return (
          <motion.span
            ...
            style={{
              background:
                i < BARS * 0.55
                  ? "linear-gradient(180deg, var(--vortex-violet-bright), var(--vortex-blue))"
                  : "oklch(1 0 0 / 0.12)",
              minHeight: 3,
            }}
            animate={
              active
                ? { height: [`${base * 0.35}%`, `${base}%`, `${base * 0.45}%`] }
                : { height: `${base * 0.4}%` }
            }
            transition={{
